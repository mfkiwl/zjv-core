...
class QarmaEngine(ppl: Boolean, static_ppl: Boolean, max_round: Int = 7) extends QarmaParamsIO {
  ...
  val engine = if (ppl) {
    if (static_ppl)
      Module(new QarmaEnginePPStaticRound(fixed_round = max_round))
    else
      Module(new QarmaEnginePP(max_round = max_round))
  } else Module(new QarmaEngineOneStage(max_round = max_round))
