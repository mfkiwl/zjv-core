...
//   val read =
//     Mux(is_alloc_reg, refill_buf.asUInt, Mux(ren_reg, rdata, rdata_buf))

//   hit := v(idx_reg) && rmeta.tag === tag_reg

//   // Read Mux
//   io.cpu.resp.bits.data := Vec.tabulate(nWords)(i =>
...
//   val wmask = Mux(
//     !is_alloc,
//     (cpu_mask << Cat(off_reg, 0.U(byteOffsetBits.W))).zext,
//     SInt(-1)
//   )
//   val wdata = Mux(
//     !is_alloc,
...
//       case (mem, i) =>
//         val data = Vec.tabulate(wBytes)(k =>
//           wdata(i * xlen + (k + 1) * 8 - 1, i * xlen + k * 8)
//         )
//         mem.write(
//           idx_reg,
//           data,
//           wmask((i + 1) * wBytes - 1, i * wBytes).toBools
//         )
...
//     Vec.tabulate(dataBeats)(i =>
//       read((i + 1) * nastiXDataBits - 1, i * nastiXDataBits)
//     )(write_count),
//     None,
//     write_wrap_out
//   )
