...
//   io.mem.req.valid := s3_valid && (state === s_memReadReq || state === s_memReadResp)
...
//       when(io.in.flush) {
//         state := s_flush
//       }.elsewhen(!s3_hit && s3_valid && !io.in.flush) {
//         state := s_memReadReq
//       }
//     }
//     is(s_memReadReq) {
...
//       write_meta(i).meta := DontCare
//       write_meta(i).tag := DontCare
//     }
//     for (i <- 0 until nWays) {
//       metaArray(i).write(flush_counter.value, write_meta(i))
//     }
//     flush_counter.inc()
//   }
